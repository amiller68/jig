...
use crate::git::conventional::CommitType;
...
impl Default for ConventionalCommitsConfig {
    fn default() -> Self {
        Self {
            types: CommitType::ALL.iter().map(|t| t.as_str().to_string()).collect(),
            require_scope: false,
            scopes: vec![],
            allow_breaking: true,
...
impl ConventionalCommitsConfig {
    pub fn to_validation_config(&self) -> crate::git::conventional::ValidationConfig {
        let allowed_types: Vec<CommitType> = self
            .types
            .iter()
            .filter_map(|s| s.parse().ok())
            .collect();

        crate::git::conventional::ValidationConfig {
            allowed_types,
